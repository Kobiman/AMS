    ...
    public class AgentService : IAgentService
    {
        ...
        public async Task<IEnumerable<AgentReportDto>> GetAgentReport(string period)
        {
            ...
            return sales.Select(x => {
                var payinAmount = payout_payin.Where(y => y.EntryDate == x.EntryDate).Sum(y => y.PayinAmount);
                var payoutAmount = payout_payin.Where(y => y.EntryDate == x.EntryDate).Sum(y => y.PayoutAmount);
